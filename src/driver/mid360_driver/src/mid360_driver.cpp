...
#pragma pack()
    ...
    void Mid360Driver::setTransform(const Transform &new_transform) {
        ...
        while (is_running.load(std::memory_order_relaxed)) {
            ...
            if (header.data_type == DataType::kLivoxLidarCartesianCoordinateHighData) {
                ...
                for (size_t i = 0; i < header.dot_num; ++i) {
                    ...
                    // 应用坐标变换
                    transform.transformPoint(point.x, point.y, point.z);
                    
                    // 应用距离筛选
                    if (transform.filterPointByDistance(point.x, point.y, point.z)) {
                ...
                for (size_t i = 0; i < header.dot_num; ++i) {
                    ...
                    point.x = static_cast<float>(radius * sin_theta * cos_phi);
                    point.y = static_cast<float>(radius * sin_theta * sin_phi);
                    point.z = static_cast<float>(radius * cos_theta);
                    point.intensity = raw_point.reflectivity;
    ...
    asio::awaitable<void> Mid360Driver::receive_imu() {
        ...
        while (is_running.load(std::memory_order_relaxed)) {
            ...
            // 对IMU数据应用旋转变换（只旋转不平移）
            transform.transformVector(imu_msg.angular_velocity_x, imu_msg.angular_velocity_y, imu_msg.angular_velocity_z);
            transform.transformVector(imu_msg.linear_acceleration_x, imu_msg.linear_acceleration_y, imu_msg.linear_acceleration_z);
